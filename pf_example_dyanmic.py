...
def update(frame):
    ...
    # Wrap scalars in lists for single points
    robot_plot.set_data([robot_position[0]], [robot_position[1]])
    obstacles_plot.set_data(obstacles[:, 0], obstacles[:, 1])

    # Quiver expects 2D arrays for positions and vectors
    quiver_robot.set_offsets([robot_position])  # 2D array: [[x, y]]
    quiver_robot.set_UVC([force[0]], [force[1]])  # wrap in lists
